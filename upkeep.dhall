...
let installNix = Step::
  ...
  , `with` = Some WithClause::{ extra_nix_config = Some
                                  ''
...
let gitDiff = Step::{ run = Some "git diff", working-directory = Some "./nixpkgs" }
let allowUnfree =
  Step::{ run = Some
              ''
...
{ jobs =
  ...
  , jax = Job::
    { steps =
      ...
      , canary
      , Step::{ name = Some "Run custom update-script"
              , run = Some
                  ''
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A python3Packages.jax"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ name = Some "Create PR"
              , working-directory = Some "./nixpkgs"
              , run = Some
                  ''
                  GH_TOKEN=$GH_TOKEN \
  ...
  , julia_17-bin = Job::
    { steps =
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A julia_17-bin"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ name = Some "Create PR"
              , env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
  ...
  , matplotlib = Job::
    { steps =
      ...
      , checkoutNixpkgsUpkeep
      , checkoutNixpkgs
      , Step::{ name = Some "Check current package version before update script"
              , run = Some
                  ''
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A python3Packages.matplotlib"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ name = Some "Create PR"
              , env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
  ...
  , plexamp = Job::
    { steps =
      ...
      , checkoutNixpkgs
      , allowUnfree
      -- TODO: use checkVersion "python3Packages.plexamp"
      , Step::{ name = Some "Check current package version before update script"
              , run = Some
                  ''
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A plexamp"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
              , name = Some "Create PR"
              , run = Some
                  ''
                  PACKAGE="plexamp" \
  ...
  , spotify = Job::
    { steps =
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A spotify"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
              , name = Some "Create PR"
  ...
  , vscode = Job::
    { steps =
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A vscode"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
              , name = Some "Create PR"
  ...
  , vscodium = Job::
    { steps =
      ...
      , checkoutNixpkgsUpkeep
      , checkoutNixpkgs
      -- TODO: use checkVersion "vscodium"
      , Step::{ name = Some "Check current package version before update script"
              , run = Some
                  ''
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A vscodium"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
              , name = Some "Create PR"
  ...
  , wandb = Job::
    { steps =
      ...
      , gitDiff
      , Step::{ run = Some "nix-build -A python3Packages.wandb"
              , working-directory = Some "./nixpkgs"
              }
      , Step::{ env = Some { GH_TOKEN = "\${{ secrets.GH_TOKEN }}" }
              , name = Some "Create PR"
...
, name = "upkeep"
, on = { push.branches = [ "main" ], schedule = [ { cron = "0 0,12 * * *" } ] }
}
